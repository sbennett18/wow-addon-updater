...
def curse(addonpage):
    ...
    try:
        ...
        indexOfZiploc = (
            contentString.find("download__link") + 22
        )  # Will be the index of the first char of the url
        endQuote = contentString.find(
            '"', indexOfZiploc
...
def curseDatastore(addonpage):
    try:
        ...
        projectPage = (
            contentString[indexOfProjectPageURL:endOfProjectPageURL] + "/files"
        )
        ...
        indexOfZiploc = (
            contentString.find(
                '<a class="button tip fa-icon-download icon-only" href="/', startOfTable
            )
            + 55
        )
...
def convertOldCurseURL(addonpage):
    try:
        ...
        print(
            'Failed to find the current page for old URL "'
            + addonpage
            + '". Skipping...\n'
        )
        return ""
...
def getCurseVersion(addonpage):
    ...
    try:
        ...
        contentString = str(page.content)
        indexOfVer = (
            contentString.find("file__name full") + 17
        )  # first char of the version string
        endTag = contentString.find(
            "</span>", indexOfVer
        )  # ending tag after the version string
...
def getCurseProjectVersion(addonpage):
    try:
        ...
        if page.status_code == 404:
            ...
            page = requests.get(
                page.url + "/files"
            )  # page.url refers to the place where the last one redirected to
            page.raise_for_status()  # Raise an exception for HTTP errors
        contentString = str(page.content)
        startOfTable = contentString.find("project-file-list-item")
        indexOfVer = (
            contentString.find('data-name="', startOfTable) + 11
        )  # first char of the version string
        endTag = contentString.find(
            '">', indexOfVer
        )  # ending tag after the version string
...
def wowinterface(addonpage):
    ...
    try:
        ...
        contentString = str(page.content)
        indexOfZiploc = (
            contentString.find('Problems with the download? <a href="') + 37
...
def getWowinterfaceVersion(addonpage):
    try:
        ...
        contentString = str(page.content)
        indexOfVer = (
            contentString.find('id="version"') + 22
        )  # first char of the version string
        endTag = contentString.find(
            "</div>", indexOfVer
        )  # ending tag after the version string
