...
class AddonUpdater:
    def __init__(self):
        ...
        try:
            ...
            self.INSTALLED_VERS_FILE = config["WOW ADDON UPDATER"][
                "Installed Versions File"
            ]
            self.AUTO_CLOSE = config["WOW ADDON UPDATER"][
                "Close Automatically When Completed"
            ]
        except Exception:
            print(
        ...
        with open(self.ADDON_LIST_FILE, "r") as fin:
            for line in fin:
                ...
                if not currentVersion == installedVersion:
                    ...
                    print(
                        addonName + " version " + currentVersion + " is up to date.\n"
                    )
                    current_node.append("Up to date")
                uberlist.append(current_node)
        if self.AUTO_CLOSE == "False":
            col_width = (
                max(len(word) for row in uberlist for word in row) + 2
            )  # padding
            print(
                "".join(
                    word.ljust(col_width) for word in ("Name", "Iversion", "Cversion")
                )
            )
            for row in uberlist:
    ...
    def getInstalledVersion(self, addonpage, subfolder):
        ...
        try:
            if subfolder:
                return installedVers["Installed Versions"][
                    addonName + "|" + subfolder
    ...
    def setInstalledVersion(self, addonpage, subfolder, currentVersion):
        ...
        if subfolder:
            installedVers.set(
                "Installed Versions", addonName + "|" + subfolder, currentVersion
            )  # Keep subfolder info in installed listing
        else:
...
def main():
    if isfile("changelog.txt"):
        downloadedChangelog = requests.get(
            "https://raw.githubusercontent.com/kuhnerdm/wow-addon-updater/master/changelog.txt"
        ).text.split("\n")
    ...
    if downloadedChangelog != presentChangelog:
        print(
            "A new update to WoWAddonUpdater is available! Check it out at https://github.com/kuhnerdm/wow-addon-updater !"
        )
